...
async function onRequest(context2) {
  ...
  for (const [key, val] of context2.request.headers.entries()) {
    if (!["host", "cf-connecting-ip", "cf-ray"].includes(key.toLowerCase())) {
      headers.set(key, val);
    }
  }
  let body = void 0;
  if (!["GET", "HEAD"].includes(context2.request.method)) {
    body = await context2.request.arrayBuffer();
  }
  const response = await fetch(targetUrl, { method: context2.request.method, headers, body });
...
// ../.wrangler/tmp/pages-grOuzr/functionsRoutes-0.3930234354279285.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
